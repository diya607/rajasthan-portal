...
# Declare where resumes will be saved on your disk
UPLOAD_FOLDER = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'static/uploads/resumes')
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['MAX_CONTENT_LENGTH'] = 5 * 1024 * 1024  # Limit file size to 5MB max
...
# ── Dynamic Local State Storage (Production Demo Fallbacks) ──────────────────
DEMO_STUDENTS = [
...
# ── AUTHENTICATION REDIRECT GUARD DECORATOR ─────────────────────────────────
def login_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if "logged_in" not in session:
            flash("Please log in to access this secure portal resource.", "error")
...
def login():
    if request.method == "POST":
        ...
        if captcha.upper() != "X4P2QR":
            flash("Invalid CAPTCHA sequence entered. Please try again.", "error")
        ...
        if cur:
            try:
                ...
                if user and user["password"] == password:
                    ...
                    flash(f"Welcome back, {session['user_display']}.", "success")
            ...
            if username == "admin" and password == "Rajasthan@2026":
                ...
                flash("Login successful! Welcome, Arjun Kumar (Simulation Instance).", "success")
                return redirect(url_for("dashboard"))
                
        flash("Invalid Employee ID / SSO ID or password context.", "error")
...
# ── TEMPLATE-MATCHED UI PAGE ENDPOINTS ───────────────────────────────────────
    ...
    if request.method == "POST":
        ...
        if 'resume' in request.files:
            ...
            if file and file.filename != '' and allowed_file(file.filename):
                # secure_filename sanitizes the name to prevent directory traversal exploits
                filename = secure_filename(f"{datetime.now().strftime('%Y%m%d%H%M%S')}_{file.filename}")
                file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
                resume_filename = filename  # This string goes to your database
        
        if cur:
            try:
                # 2. Updated SQL query to include the resume column
                cur.execute(
                    "INSERT INTO students (name, branch, cgpa, skills, email, phone, resume_path, status) "
                    "VALUES (%s, %s, %s, %s, %s, %s, %s, 'Searching')",
                    (
                        data.get("name"), data.get("branch"), data.get("cgpa"),
                        data.get("skills"), data.get("email"), data.get("phone"),
                        resume_filename  # Saved filename reference string
                    ),
                )
                mysql.connection.commit()
                cur.close()
                flash("Student registered with resume successfully!", "success")
            except Exception as e:
                flash(f"Database submission error: {str(e)}", "error")
        else:
            # Fallback for your runtime memory mode
            new_student = {
            ...
            flash("Student added cleanly to runtime memory with mock resume link.", "success")
...
# ── GLOBAL SYSTEM ERROR HANDLING BOUNDARIES ──────────────────────────────────

@app.errorhandler(404)
def not_found(e):
    return "<h1>404 Page Not Found</h1><p>The layout factory route could not map accurately.</p>", 404
...
if __name__ == "__main__":
    ...
    print("  Running Flat Production Architecture Environment")
    print("=" * 60)
